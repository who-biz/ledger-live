  ...
  return {
    ...
    height:
      transaction.height !== undefined
        ? new BigNumber(transaction.height)
        : undefined,
    id: transaction.id,
    offset:
      transaction.offset !== undefined
        ? Buffer.from(transaction.offset, "hex")
        : undefined,
    proof:
      transaction.proof !== undefined
        ? Buffer.from(transaction.proof, "hex")
        : undefined,
...
export const toTransactionRaw = (transaction: Transaction): TransactionRaw => {
  ...
  return {
    ...
    height:
      transaction.height !== undefined
        ? transaction.height.toFixed()
        : undefined,
    id: transaction.id,
    offset:
      transaction.offset !== undefined
        ? transaction.offset.toString("hex")
        : undefined,
    proof:
      transaction.proof !== undefined
        ? transaction.proof.toString("hex")
        : undefined,
    privateNonceIndex: transaction.privateNonceIndex,
    transactionResponse: transaction.transactionResponse,
    useDefaultBaseFee: transaction.useDefaultBaseFee,

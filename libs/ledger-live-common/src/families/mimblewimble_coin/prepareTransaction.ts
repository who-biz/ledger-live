...
export default async (
  ...
  let currentIdentifier: Identifier = (account as MimbleWimbleCoinAccount)
  ...
  const privateNonceIndex = (await mimbleWimbleCoin.applyOffsetToTransaction(
    slate.offset
  )) as number;
  ...
  return {
    transactionData:
      serializedSlate instanceof Buffer
        ...
        : JSONBigNumber.stringify(serializedSlate),
    height: slate.height!.toFixed(),
    id: slate.id,
    offset: slate.offset.toString("hex"),
    proof: slate.outputs.length
      ? slate.outputs[0].proof.toString("hex")
      : undefined,
    privateNonceIndex,
  };
};

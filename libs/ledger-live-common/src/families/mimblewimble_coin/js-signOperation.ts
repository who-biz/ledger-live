  ...
  return new Observable((
    ...
    (async () => {
      try {
        ...
        const slate = new Slate(account.currency, transaction.amount, fee, transaction.sendAsFile ? transaction.height! : tipHeight.plus(1), new BigNumber(0), null, usePaymentProof ? account.freshAddresses[0].address : null, usePaymentProof ? recipient : null, supportedSlateVersions);
        if(transaction.sendAsFile) {

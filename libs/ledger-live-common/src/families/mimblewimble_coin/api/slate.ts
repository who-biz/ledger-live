  ...
  private async getPaymentProofMessage(): Promise<Buffer> {
    ...
    switch(this.cryptocurrency.id) {
      ...
      case "grin_testnet":
        ...
        const buffer = Buffer.alloc(Uint64Array.BYTES_PER_ELEMENT + Crypto.COMMITMENT_LENGTH + Crypto.ED25519_PUBLIC_KEY_LENGTH);
        Uint64Array.writeBigEndian(buffer, this.amount, 0);
        excess.copy(buffer, Uint64Array.BYTES_PER_ELEMENT);
        senderPaymentProofPublicKey.copy(buffer, Uint64Array.BYTES_PER_ELEMENT + Crypto.COMMITMENT_LENGTH);
        return buffer;
      default:
        throw new MimbleWimbleCoinInvalidParameters("Invalid cryptocurrency");
    }
